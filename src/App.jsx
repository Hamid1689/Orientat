...
const C = {
  ...
  cardHover:   "#F0F3FA",        // card hover
  purpleLight: "#EEF0FA",        // light purple tint
  border:      "#D8DCF0",        // borders
  textDark:    "#1A1640",        // main text
  textMid:     "#3D3A6B",        // secondary text
  textMuted:   "#7B7A9D",        // muted text
};

/* ══ LOGO — with purple circle bg so it stands out on white ══ */
const Logo = ({ size = 38 }) => (
  <svg width={size} height={size} viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg">
    {/* Circle background so logo pops on any bg */}
...
function fmtFee(n){return n.toLocaleString("ru-RU")+" сом";}
...
export default function OrientAT(){
  ...
  const sendMsg=async()=>{const cur=msgs[lang];if(!input.trim()||loading)return;const um={role:"user",content:input.trim()};const nm=[...cur,um];setMsgs(p=>({...p,[lang]:nm}));setInput("");setLoading(true);try{const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,system:SYSTEM_BASE+"\n\n"+t.systemLang,messages:nm.map(m=>({role:m.role,content:m.content}))})});const data=await res.json();setMsgs(p=>({...p,[lang]:[...nm,{role:"assistant",content:data.content?.[0]?.text||t.contacts}]}));}catch{setMsgs(p=>({...p,[lang]:[...nm,{role:"assistant",content:"⚠️ "+t.contacts}]}));}setLoading(false);};
        ...
        {screen==="home"&&(
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(270px,1fr))",gap:16}}>
              {faculties.map(f=>{
                const lo=minFee(f.id),hi=maxFee(f.id);
                return(
                  <div key={f.id} onClick={()=>{setDetailFaculty(f.id);setScreen("faculty");}}
                    ...
                    {/* Desc — flex-grow so it pushes price to bottom */}
                    ...
                    <div style={{borderTop:"1px solid rgba(255,255,255,0.12)",paddingTop:12,display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:"auto"}}>
                      <div>
                        ...
                        <div style={{fontSize:18,fontWeight:900,color:f.color===C.purple?C.white:f.color,marginTop:4}}>{lo===hi?fmtFee(lo):`от ${fmtFee(lo)}`}</div>
                      </div>
        ...
        {screen==="faculty"&&detailFaculty&&(()=>{
          ...
          return(
            <div style={{maxWidth:740,margin:"0 auto"}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {progs.map(([key,p],idx)=>(
                  <div key={key} style={{background:C.purple,border:`1px solid ${C.border}`,borderLeft:`3px solid ${idx%2===0?C.red:C.purple}`,borderRadius:10,padding:"14px 16px",display:"flex",gap:12,alignItems:"flex-start",transition:"background 0.15s"}}
          ...
          <div style={{maxWidth:580,margin:"0 auto"}}>
            <div style={{marginBottom:22}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                ...
                <button onClick={()=>setScreen("home")} style={{fontSize:11,color:C.textMuted,background:"none",border:`1px solid ${C.border}`,cursor:"pointer",padding:"3px 10px",borderRadius:4,color:C.textDark}}>{t.exit}</button>
              </div>
        ...
        {screen==="result"&&(
          <div style={{maxWidth:640,margin:"0 auto"}}>
                ...
                <div key={f.id} onClick={()=>{setDetailFaculty(f.id);setScreen("faculty");}}
                  ...
                  <div style={{display:"flex",gap:11,alignItems:"center"}}>
                    ...
                    <div style={{textAlign:"right",flexShrink:0}}>
                      <div style={{fontSize:14,fontWeight:800,color:f.color}}>{lo!==hi?"от ":""}{fmtFee(lo)}</div>
            ...
            <div style={{display:"flex",gap:10,marginTop:14,flexWrap:"wrap"}}>
              <button onClick={resetQuiz} style={{padding:"11px 20px",borderRadius:8,border:`1.5px solid ${C.border}`,background:"transparent",color:C.white,fontWeight:600,fontSize:13,cursor:"pointer"}}>{t.tryAgain}</button>
